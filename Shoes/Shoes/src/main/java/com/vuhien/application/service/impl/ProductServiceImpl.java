...
public class ProductServiceImpl implements ProductService {
    ...
    public void createSizeCount(CreateSizeCountRequest createSizeCountRequest) {
        ...
        Optional<ProductSize> productSizeOld = productSizeRepository.getProductSizeByProductIdAndSize(createSizeCountRequest.getProductId(), createSizeCountRequest.getSize());

        ProductSize productSize = new ProductSize();
        productSize.setProductId(createSizeCountRequest.getProductId());
        productSize.setSize(createSizeCountRequest.getSize());
        if(productSizeOld.isPresent()){
            productSize.setQuantity(productSizeOld.get().getQuantity() + createSizeCountRequest.getCount());
        }else {
    ...
    @Override
    public String getSizeAndQuantityOfOneProduct(String id) {
        StringBuilder sb = new StringBuilder();
        // Lấy danh sách kích thước của sản phẩm
        List<Integer> sizes = productSizeRepository.findAllSizeOfProduct(id);
        for(Integer size: sizes){
         // Lấy số lượng tồn kho của sản phẩm theo kích thước
            sb.append(size + ": " + productSizeRepository.getProductSizeByProductIdAndSize(id, size).get().getQuantity() + "<br />");
        }
        return sb.toString();
    }
}
